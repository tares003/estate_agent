...
// comment, and a "may we publish this as a testimonial?" toggle. The free-text
// comment can carry personal data, and the submission is a public personal-data
// form, so it MUST carry an explicit GDPR-consent affirmation (`gdpr_consent`,
// G5 / master spec §S.7). The publish toggle is a separate publishing consent.

describe('feedbackSubmissionSchema', () => {
  it('accepts a rating with an optional comment + publish toggle + consent', () => {
  ...
  it('requires the GDPR-consent affirmation (G5)', () => {
    expect(feedbackSubmissionSchema.safeParse({ rating: 5 }).success).toBe(false);
    expect(feedbackSubmissionSchema.safeParse({ rating: 5, gdpr_consent: false }).success).toBe(
      false,
    );
    expect(feedbackSubmissionSchema.safeParse({ rating: 5, gdpr_consent: true }).success).toBe(
      true,
    );
  });

  it('coerces the rating from a form string', () => {
    expect(feedbackSubmissionSchema.parse({ rating: '4', gdpr_consent: true }).rating).toBe(4);
  });

  it('rejects a rating outside 1–5 or non-integer', () => {
    for (const rating of [0, 6, 4.5, -1]) {
      expect(feedbackSubmissionSchema.safeParse({ rating, gdpr_consent: true }).success).toBe(
        false,
      );
    }
  });
  ...
  it('defaults publishAsTestimonial to false and the comment to absent', () => {
    const parsed = feedbackSubmissionSchema.parse({ rating: 3, gdpr_consent: true });
  ...
  it('treats a blank comment as no comment', () => {
    expect(
      feedbackSubmissionSchema.parse({ rating: 3, comment: '   ', gdpr_consent: true }).comment,
    ).toBeUndefined();
  });
  ...
  it('rejects an over-long comment', () => {
    ...
    expect(
      feedbackSubmissionSchema.safeParse({ rating: 3, comment: long, gdpr_consent: true }).success,
    ).toBe(false);
  });
});
